    ...
    stage('Launch on Staging Server') {
      ...
      steps {
        ...
        configFileProvider(
          [configFile(fileId: 'maven-settings', variable: 'MAVEN_SETTINGS')]) {
          sh 'mvn -s $MAVEN_SETTINGS deploy'
          sh 'mvn org.apache.maven.plugins:maven-dependency-plugin:2.1:get -DrepoUrl=http://192.168.1.14:8081/artifactory/libs-snapshot-local/ -Dartifact=org.springframework.samples:spring-petclinic:${VERSION}'
